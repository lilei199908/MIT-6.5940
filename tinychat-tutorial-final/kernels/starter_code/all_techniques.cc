    ...
    for (int row = 0; row < m; row++) {
        for (int col = mat_args->start_j; col < mat_args->end_j; col++) {
            ...
            for (int q = 0; q < num_block; q += 4) {
                ...
                __m256i w_0 = _mm256_and_si256(raw_w, lowMask);
                __m256i w_0_next = _mm256_and_si256(raw_w_next, lowMask);
                __m256i w_128 = _mm256_and_si256(_mm256_srli_epi16(raw_w, 4), lowMask);
                __m256i w_128_next = _mm256_and_si256(_mm256_srli_epi16(raw_w_next, 4), lowMask);
                // TODO: apply zero_point to weights and convert the range from (0, 15) to (-8, 7)
...
void MatmulOperator::mat_mul_all_techniques(struct matmul_params *params) {
    ...
    for (int j = 0; j < num_thread; j++)
    {
        ...
        threads_args[j].end_j = std::min((j + 1) * (n / num_thread), n);
